...
/*Add by Jim*/
#define CRASH_RECC_FLASH_ADDR  (0xC0000)
#define CRASH_RECC_MAX_REC     (8)

typedef struct
{
	uint8_t             lastRec;
	uint8_t             resetinfo[CRASH_RECC_MAX_REC];
	uint16_t            ext_resetinfo[CRASH_RECC_MAX_REC];
	HalCrashInfoType    crashRec[CRASH_RECC_MAX_REC];
}CrashRecc;

#define hal_ResetInfoWasCrash(info) \
  (((1 << (info)) & RESET_CRASH_REASON_MASK) != 0U)
