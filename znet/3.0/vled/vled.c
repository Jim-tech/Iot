...
static uint8_t vled_state[VLED_NUMBER] = {0, 0};

void vled_UpdateDisplay()
{
    uint8_t        i;
    uint8_t        led;
    unsigned char *pbitmap = NULL;
    char          *label[VLED_NUMBER] = {"LED0", "LED1"};
    ...
    for (i = 0; i < VLED_NUMBER; i++) {
        ...
        if (vled_state[led]) {
            pbitmap = lightOnBitMap;
        } else {
            pbitmap = lightOffBitMap;
        }
        
        GLIB_drawBitmap(&glibContext,
                        LIGHT_X_POSITION + glibContext.pDisplayGeometry->xSize/2 * i,
                        LIGHT_Y_POSITION,
                        LIGHT_BITMAP_WIDTH,
                        LIGHT_BITMAP_HEIGHT,
                        pbitmap);
        ...
        GLIB_drawString(&glibContext, 
                        label[led],
                        strlen(label[led]) + 1,
                        LIGHT_X_POSITION + glibContext.pDisplayGeometry->xSize/2 * i + LIGHT_BITMAP_WIDTH/2 - 8,
                        LIGHT_Y_POSITION + LIGHT_BITMAP_HEIGHT + 8, 
                        0);
...
void vled_Toggle(uint8_t led)
{
    if (led >= VLED_NUMBER) {
        return;
    }

    vled_state[led] = 1 - vled_state[led];
    vled_UpdateDisplay();        
}

void emberAfVirtualLedInitCallback()
{
  GRAPHICS_Init();
