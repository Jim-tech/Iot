            ...
            _IN_OPT_ uint8_t channel)
{
    ...
    ret = wifi_connect(ssid, passwd, connection_timeout_ms);
    if (0 != ret) {
        dbg_error("WiFi connect failed, ssid=[%s] passwd=[%s]", ssid, passwd);
        return ret;
    }
                   ...
                   _IN_ unsigned int timeout_ms)
{
    ...
    dbg_trace("ipaddr=0x%08x port=%d", (unsigned int)(udpaddr.ipaddr), udpaddr.port);
