...
static const char nameStrings[] = "R0\0R1\0R2\0R3\0"
                                  ...
                                  "Ret4\0Ret5\0Ret6\0Ret7\0Ret8\0Ret9\0Dat0\0Dat1\0";
...
/*Add by Jim*/
CrashRecc g_crashRecord;

void emberCrashRecordInit(uint8_t port)
{
	halInternalResetWatchDog();
	if (0 != halAppBootloaderInit()) {
		emberSerialPrintfLine(port, "BTL Interface Init failed");
	} else {
		halAppBootloaderReadRawStorage(CRASH_RECC_FLASH_ADDR, (uint8_t *)&g_crashRecord, sizeof(CrashRecc));
		if (g_crashRecord.lastRec >= CRASH_RECC_MAX_REC) {
			g_crashRecord.lastRec = 0;
			emberSerialPrintfLine(port, "Crash record init");
			memset(&g_crashRecord, 0, sizeof(g_crashRecord));
			halAppBootloaderEraseRawStorage(CRASH_RECC_FLASH_ADDR, FLASH_PAGE_SIZE);
			halAppBootloaderWriteRawStorage(CRASH_RECC_FLASH_ADDR, (uint8_t *)&g_crashRecord, sizeof(CrashRecc));
		}
	}
}

void emberCrashRecordSave(uint8_t port)
{
	uint8_t index = g_crashRecord.lastRec;

	halInternalResetWatchDog();
	g_crashRecord.resetinfo[index] = halGetResetInfo();
	g_crashRecord.ext_resetinfo[index] = halGetExtendedResetInfo();
	memcpy(&(g_crashRecord.crashRec[index]), &halResetInfo.crash, sizeof(HalCrashInfoType));
	g_crashRecord.lastRec = (g_crashRecord.lastRec + 1) % CRASH_RECC_MAX_REC;
	halAppBootloaderEraseRawStorage(CRASH_RECC_FLASH_ADDR, FLASH_PAGE_SIZE);
	halAppBootloaderWriteRawStorage(CRASH_RECC_FLASH_ADDR, (uint8_t *)&g_crashRecord, sizeof(CrashRecc));
	emberSerialPrintfLine(port, "Crash record saved");
}

void halPrintCrashSummaryByRec(uint8_t port, uint8_t index)
{
  HalCrashInfoType *c = &(g_crashRecord.crashRec[index]);
  uint32_t sp, stackBegin, stackEnd, size, used;
  uint16_t pct;
  uint8_t *mode;
  const char *stack;
...
void halPrintCrashDetailsByRec(uint8_t port, uint8_t index)
{
  HalCrashInfoType *c = &(g_crashRecord.crashRec[index]);
//  uint16_t reason = halGetExtendedResetInfo();
  uint16_t reason = g_crashRecord.ext_resetinfo[index];
  uint8_t bit;
  const uint8_t numFaults = sizeof(cfsrBits) / sizeof(cfsrBits[0]);
...
// Output an array of 32 bit values, 4 per line, each preceded by its name.
void halPrintCrashDataByRec(uint8_t port, uint8_t index)
{
  uint32_t *data = &(g_crashRecord.crashRec[index].R0);
  char const *name = nameStrings;
  char const *separator;
  uint8_t i = 0;
...
uint8_t emberGetMaxCrashRecordNum()
{
	return CRASH_RECC_MAX_REC;
}

uint8_t emberGetLastCrashRecordIndex()
{
	return (g_crashRecord.lastRec + CRASH_RECC_MAX_REC - 1) % CRASH_RECC_MAX_REC;
}

void emberCrashRecordDump(uint8_t port, uint8_t index)
{
	uint8_t reset_info = g_crashRecord.resetinfo[index];
	if (0xFF != reset_info && hal_ResetInfoWasCrash(reset_info)) {
		halPrintCrashSummaryByRec(port, index);
		halPrintCrashDetailsByRec(port, index);
		halPrintCrashDataByRec(port, index);
	}
}
/*End add*/
