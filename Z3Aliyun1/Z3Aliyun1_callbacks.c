...
//    	  }
        ...
        dbg_error("Cloud disconnected! reset...");

//        while (!wifi_is_connected()) {
//        	wifi_connect_withsaveddata(15000);
//
//        	RTOS_ERR err;
...
bool emberAfStackStatusCallback(EmberStatus status)
{
    ...
    if ((0 != strlen(wifi_data.ssid)) && (0 != strlen(wifi_data.passwd))) {
        ...
        ipc_SendAliyunCmdCommon(IPC_ALIYUN_WIFI_PROVISION_CMD, NULL, 0);
    }
...
void emberAfPluginMicriumRtosAppTask1MainLoopCallback(void *p_arg)
{
	...
	switch (pMsg->msgCmd)
	{
	case IPC_ALIYUN_WIFI_PROVISION_CMD:
		msgq_FreeBuffer(pMsg);
		pMsg = NULL;
	...
	case IPC_ALIYUN_START_CMD:
        {
            AliyunStartCmdPara *pdata = (AliyunStartCmdPara *)(pMsg->data);
            while (1) {
                if (0 == wifi_connect(pdata->ssid, pdata->passwd, 10000)) {
                    break;
                }
