...
static void wifi_endpoint_status_change(uint8_t endpoint, bool up)
{
    ...
    for (i = 0; i < MAX_EP_SIMULTANEOUS_NUM; i++) {
        if (true == g_wifi_ep_state[i].used) {
            dbg_trace("index(%d) ep(%d)", i, g_wifi_ep_state[i].endpoint);
        }
    }
    return;
}
...
{
    uint16_t                msg_length;
    uint16_t                pskey = FLASH_PS_KEY_CLIENT_SSID;
    char                    ipstring[32] = {0};
    RTOS_ERR                err;
    ...
    while (1) {
        ...
        switch(BGLIB_MSG_ID(g_uartrx_buffer)) {
            ...
            case wifi_rsp_flash_ps_load_id:
                dbg_trace("recv %s result=%d", DESC(wifi_rsp_flash_ps_load_id), pck->rsp_flash_ps_load.result);
                if (wifi_err_success == pck->rsp_flash_ps_load.result) {
                    if (FLASH_PS_KEY_CLIENT_SSID == pskey) {
                        dbg_trace("ssid len=%d [%s]", pck->rsp_flash_ps_load.value.len, (char *)pck->rsp_flash_ps_load.value.data);
                        ...
                        dbg_trace("passwd len=%d [%s]", pck->rsp_flash_ps_load.value.len, (char *)pck->rsp_flash_ps_load.value.data);
            ...
            case wifi_rsp_flash_ps_erase_all_id:
                dbg_trace("recv %s %d", DESC(wifi_rsp_flash_ps_erase_all_id), pck->rsp_flash_ps_erase_all.result);
            ...
            case wifi_evt_sme_interface_status_id:
                dbg_trace("WiFi set interface up");
            ...
            case wifi_evt_endpoint_status_id:
                dbg_trace("recv %s active=%d", DESC(wifi_evt_endpoint_status_id), pck->evt_endpoint_status.active);
            ...
            case wifi_rsp_endpoint_send_id:
                dbg_trace("recv %s result=%d", DESC(wifi_rsp_endpoint_send_id), pck->rsp_endpoint_send.result);
            ...
            case wifi_evt_tcpip_udp_data_id:
                ...
                wifi_endpoint_data_enqueue(pck->evt_tcpip_udp_data.endpoint, 
...
int wifi_reset()
{
    ...
    while (1) {
        ...
        if (halCommonGetInt64uMillisecondTick() - start > WIFI_DEFAULT_TIMEOUT) {
...
int wifi_connect(const char *pssid, const char *ppasswd, int timeout_ms)
{
    ...
    dbg_error("connect ssid[%s] password[%s]", pssid, ppasswd);
    ...
    if (WLAN_ERR_NONE != ret) {
        dbg_error("set passwd fail");
        return ret;
    }

    ret = wifi_connect_ssid((char *)pssid);
    if (WLAN_ERR_NONE != ret) {
        dbg_error("connect fail");
        return ret;
    }
    ...
    if (0 != ret) {
        dbg_error("set tls auth mode fail");
        return ret;
    }
...
int wifi_tcpip_disconnect(uint8_t endpoint)
{
    ...
    *(uint8_t *)wifi_oper_ctrl.input = endpoint;
    ...
    wifi_endpoint_status_change(endpoint, false);
    OSMutexPost(&g_wifi_mutex, OS_OPT_POST_NONE, &err);
    APP_RTOS_ASSERT_DBG((RTOS_ERR_CODE_GET(err) == RTOS_ERR_NONE), 1);
    return ret;    
}
...
int wifi_udp_read(uint8_t endpoint, uint8_t *pdata, int len, int timeout_ms, UDP_Addr *psrc)
{
    ...
    start = halCommonGetInt64uMillisecondTick();
    while (cnt < len) {
        cnt += wifi_endpoint_data_dequeue(endpoint, pdata + cnt, len - cnt);
...
int wifi_udp_write(uint8_t endpoint, uint8_t *pdata, int len, int timeout_ms, UDP_Addr *premote)
{
    ...
    dbg_trace("wifi_udp_bind ep%d to port %d", endpoint_new, g_local_udpport);
